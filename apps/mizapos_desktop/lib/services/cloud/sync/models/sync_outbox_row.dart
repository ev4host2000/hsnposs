...
class SyncOutboxRow {
  ...
  SyncPushEvent toPushEvent() {
    final changed = payloadJson['changed_fields'];
    return SyncPushEvent(
      ...
      changedFields: changed is Map
          ? Map<String, dynamic>.from(changed)
          : null,
      baseRowVersion: payloadJson['base_row_version'] is int
          ? payloadJson['base_row_version'] as int
          : int.tryParse('${payloadJson['base_row_version'] ?? ''}'),
      operationId: (payloadJson['operation_id'] ?? id).toString(),
      dictionaryVersion: payloadJson['dictionary_version']?.toString(),
      contractVersion: payloadJson['contract_version'] is int
          ? payloadJson['contract_version'] as int
          : int.tryParse('${payloadJson['contract_version'] ?? ''}'),
    );
  }
}

/// ØªØ­ÙˆÙŠÙ„ [ProductEntity] Ø¥Ù„Ù‰ payload Ø³Ø­Ø§Ø¨ÙŠ (snake_case).
