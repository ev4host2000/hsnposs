...
import 'package:mizapos_desktop/services/cloud/sync/posting/customer_payment/customer_payment_posting_pipeline.dart';
import 'package:mizapos_desktop/services/cloud/sync/customer_payment_post_payload.dart';
...
/// Executes local customer-payment post and enqueues a single `post` outbox event.
class CustomerPaymentPostLocalService {
  CustomerPaymentPostLocalService({DatabaseService? databaseService})
  ...
  Future<PostingResult> postDraft({
    ...
    try {
      await db.transaction((txn) async {
        ...
        final pipeline = CustomerPaymentPostingPipeline.create();
        ...
        envelope = customerPaymentPostPushEnvelope(
  ...
  Future<MapTransactionAggregate?> _loadDraftAggregate(
    ...
    return MapTransactionAggregate.fromParts(
      header: {
        ...
        'customer_id': paymentDoc['customerId'],
  ...
  Map<String, dynamic> _buildPostAggregate(
    ...
    return customerPaymentPostAggregate(
      id: header.id,
      ...
      rowVersion: nextRow,
      postedAt: postedAt,
      cash: customerPaymentCashJsonFromStageData(stageData),
      accounting: customerPaymentAccountingJsonFromStageData(stageData),
    );
  }
