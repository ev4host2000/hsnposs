  ...
  Future<SyncPullApplyOutcome> applyVoid(
    ...
    for (final raw in inventory) {
      ...
      await txn.insert('stockMovements', {
        ...
        'movementType': 'out',
        'quantity': qty,
        'referenceType': (m['reference_type'] ?? 'purchase_void').toString(),
        'referenceId': invoiceId,
        'movementDate': (m['movement_date'] ?? when).toString(),
        'createdBy': createdBy,
      });
    }
    ...
    for (final raw in accounting) {
      ...
      await txn.insert('partnerLedger', {
        ...
        'organizationId': header.companyId,
        'branchId': header.branchId,
        'partnerKind': (e['partner_kind'] ?? 'supplier').toString(),
        'partnerId': partnerId,
        'entryType': (e['entry_type'] ?? 'purchase_void').toString(),
        'referenceType': (e['reference_type'] ?? 'purchase').toString(),
        'referenceId': invoiceId,
  ...
  Future<void> _applyVoidCashSection(
    ...
    for (final raw in cash) {
      ...
      await txn.insert('cashTransactions', {
        ...
        'referenceType':
            (c['reference_type'] ?? 'purchase_void').toString(),
        'referenceId': invoiceId,
        'transactionDate':
