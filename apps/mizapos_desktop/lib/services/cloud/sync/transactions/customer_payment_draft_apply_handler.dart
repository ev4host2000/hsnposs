...
/// Applies customer payment draft/post aggregates locally.
class CustomerPaymentDraftApplyHandler extends TransactionApplyHandler
  ...
  Future<SyncPullApplyOutcome> applyPost(
    ...
    final result = await runPostingPipeline(
      ...
      pipeline: CustomerPaymentPostingPipeline.create(),
    );
  ...
  Future<SyncPullApplyOutcome> _upsertDraft(
    ...
    if (existing.isEmpty) {
      await txn.insert('customerPayments', row);
    } else {
      await txn.update(
        'customerPayments',
        row,
