...
import 'package:mizapos_desktop/services/cloud/auth/auth_expired_exception.dart';
...
class SyncRetryPolicy {
  ...
  static bool isPermanentError(
    ...
    if (isAuthRefreshableError(error)) return false;
    if (error is AuthExpiredException) return true;

    final normalized = (code ?? _codeFromError(error)).toLowerCase();
    if (normalized.contains('auth_expired')) return true;

    final httpStatus = statusCode ??
        _statusFromError(error) ??
        _statusFromCode(normalized);

    if (httpStatus == 403) return true;
    if (httpStatus == 401) return normalized.contains('auth_expired');
  ...
  static bool isAuthRefreshableError(Object error) {
    if (error is AuthException && error.code == 'auth_refresh_failed') {
      return true;
    }
    final normalized = _codeFromError(error).toLowerCase();
    return normalized.contains('auth_refresh_failed');
  }
