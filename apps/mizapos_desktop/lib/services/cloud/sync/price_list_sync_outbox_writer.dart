...
/// Dual-mode outbox writer for price_list (M1 Full default, Patch optional).
///
/// Loads header from `price_lists` and nested items from `price_list_items`.
class PriceListSyncOutboxWriter {
  PriceListSyncOutboxWriter._();
  ...
  static bool isPriceListEntityType(String entityType) {
    return entityType == CatalogSyncConstants.entityTypePriceList;
  }
  ...
  static Future<void> record({
    ...
    if (!isPriceListEntityType(entityType)) return;
    if (operation != 'create' &&
        operation != 'update' &&
        operation != 'delete' &&
        operation != 'patch') {
    ...
    if (operation == 'delete') {
      fullPayload ??= priceListEntityCloudPayload(
        id: entityId,
      ...
      final loaded = await _loadPriceList(
        db,
        entityId: entityId,
        organizationId: organizationId,
        branchId: branchId,
    ...
    final intent = PriceListUpdateContract.buildIntent(
  ...
  static Future<({
    ...
    return (
      cloudRowVersion: cloudRowVersion,
      snapshot: PriceListUpdateContract.decodeSnapshot(
    ...
    final items = itemRows
        .map(
          (item) => <String, dynamic>{
            ...
            'sale_price': (item['salePrice'] as num?)?.toDouble() ?? 0.0,
          },
        )
