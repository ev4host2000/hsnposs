...
/// SQLite helpers for sales-return post effects (idempotent).
class SalesReturnPostDb {
  SalesReturnPostDb._();
  ...
  static Future<bool> hasPostMovementsForReturn(
    ...
    final rows = await txn.query(
      ...
      whereArgs: ['sale_return', returnId],
      limit: 1,
    );
    return rows.isNotEmpty;
  }

  /// Inserts movement if missing; adds stock once per new movement.
  static Future<void> applyInventoryEffect(
    DatabaseExecutor txn, {
    required SalesReturnInventoryEffect effect,
  ...
  static Future<void> applyAccountingEffect(
    DatabaseExecutor txn, {
    required SalesReturnAccountingEffect effect,
    required String organizationId,
    required String branchId,
  }) async {
