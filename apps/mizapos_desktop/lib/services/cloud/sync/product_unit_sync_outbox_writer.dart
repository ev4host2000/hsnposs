...
/// Dual-mode outbox writer for product_unit (M1 Full default, Patch optional).
class UnitSyncOutboxWriter {
  UnitSyncOutboxWriter._();
  ...
  static bool isUnitEntityType(String entityType) {
    return entityType == CatalogSyncConstants.entityTypeProductUnit;
  }
  ...
  static Future<void> record({
    ...
    if (!isUnitEntityType(entityType)) return;
    if (operation != 'create' &&
        operation != 'update' &&
        operation != 'delete' &&
        operation != 'patch') {
    ...
    final intent = UnitUpdateContract.buildIntent(
  ...
  static Future<({
    ...
    return (
      cloudRowVersion: cloudRowVersion,
      snapshot: UnitUpdateContract.decodeSnapshot(
        row['cloudCatalogSnapshotJson']?.toString(),
      ),
    );
  }

  static Future<({String? name})> _loadUnit(
    DatabaseExecutor db, {
    required String entityId,
    required String organizationId,
    required String branchId,
...
typedef ProductUnitSyncOutboxWriter = UnitSyncOutboxWriter;
