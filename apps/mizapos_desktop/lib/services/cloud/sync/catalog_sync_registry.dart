...
import 'package:mizapos_desktop/services/cloud/sync/contract/price_list_update_contract.dart';
import 'package:mizapos_desktop/services/cloud/sync/contract/tax_update_contract.dart';
import 'package:mizapos_desktop/services/cloud/sync/contract/unit_update_contract.dart';
...
class CatalogSyncRegistry {
  ...
  Future<SyncPullApplyOutcome> _applyProductCategory(
    ...
    return _upsertNamedRow(
      ...
      cloudCatalogSnapshotJson: jsonEncode(
        CategoryUpdateContract.catalogSnapshotFromPayload(entry.payloadJson),
      ),
    );
  }
  ...
  Future<SyncPullApplyOutcome> _applyProductUnit(
    ...
    return _upsertNamedRow(
      ...
      cloudRowVersion: entry.rowVersion,
      cloudCatalogSnapshotJson: jsonEncode(
        UnitUpdateContract.catalogSnapshotFromPayload(entry.payloadJson),
      ),
    );
  }
  ...
  Future<SyncPullApplyOutcome> _applyTax(
    ...
    if (entry.rowVersion >= 1 &&
        await _tableHasColumn(txn, 'taxes', 'cloudRowVersion')) {
      row['cloudRowVersion'] = entry.rowVersion;
      row['cloudCatalogSnapshotJson'] = jsonEncode(
        TaxUpdateContract.catalogSnapshotFromPayload(entry.payloadJson),
      );
    }
  ...
  Future<SyncPullApplyOutcome> _applyPriceList(
    ...
    if (entry.rowVersion >= 1 &&
        await _tableHasColumn(txn, 'price_lists', 'cloudRowVersion')) {
      row['cloudRowVersion'] = entry.rowVersion;
      row['cloudCatalogSnapshotJson'] = jsonEncode(
        PriceListUpdateContract.catalogSnapshotFromPayload(entry.payloadJson),
      );
    }
  ...
  Future<SyncPullApplyOutcome> _upsertNamedRow(
    ...
    String? sortColumn,
    int? cloudRowVersion,
    String? cloudCatalogSnapshotJson,
  }) async {
    ...
    if (cloudRowVersion != null &&
        cloudRowVersion >= 1 &&
        await _tableHasColumn(txn, table, 'cloudRowVersion')) {
      row['cloudRowVersion'] = cloudRowVersion;
      if (cloudCatalogSnapshotJson != null) {
        row['cloudCatalogSnapshotJson'] = cloudCatalogSnapshotJson;
      }
    }
