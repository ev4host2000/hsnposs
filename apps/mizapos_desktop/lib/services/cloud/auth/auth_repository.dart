...
class AuthRepository {
  ...
  Future<CloudApiResponse<LoginResponse>> loginPairingRemote(
    LoginRequest request,
  ) {
    return _authApi.loginPairing(request);
  }
  ...
  Future<void> persistSession(CloudSession session) async {
    ...
    final cloudUser = session.user;
    if (cloudUser != null) {
      final loginHint = (cloudUser.email?.trim().isNotEmpty ?? false)
          ? cloudUser.email!.trim()
          : cloudUser.username.trim();
      if (loginHint.isNotEmpty) {
        await _storage.writeCloudUsername(loginHint);
      }
    }
  }
