...
import 'package:mizapos_desktop/config/opening_balance_voucher_config.dart';
...
import 'package:mizapos_desktop/models/signup_approval_mail.dart';
...
import 'package:mizapos_desktop/utils/debug_session_log.dart';
...
class AccountingService {
  ...
  CloudSecureStoragePlaceholder? _transactionOutboxStorage;

  Future<void> _ensureTransactionOutboxReady() async {
    _transactionOutboxStorage ??= CloudSecureStoragePlaceholder();
    TransactionSyncOutboxWriter.bindStorage(_transactionOutboxStorage!);
  }
  ...
  /// لم يعد يُستخدم — دخول المدير بكلمة السر المسجّلة في الفريق فقط.
  bool get ownerLoginPasswordActive => false;
  ...
  /// (بريد المشترك + كلمة السر المحفوظة محلياً بعد أول تسجيل دخول للقسيمة).
  Future<bool> ensureLocalOwnerForSubscription() async {
    ...
    final emailOwner = await db.rawQuery(
      '''
    ...
    if (emailOwner.isNotEmpty) return true;

    final hash = pwdHash ?? offlineHash;
    final bootstrapOwner = await db.rawQuery(
      '''
      SELECT id FROM users
      WHERE organizationId = ?
        AND lower(trim(role)) = 'owner'
      ...
      LIMIT 1
      ''',
      [orgId, bootstrapOwnerUsername],
    );
    if (bootstrapOwner.isNotEmpty && PasswordCrypto.looksLikeBcrypt(hash)) {
      await db.update(
        'users',
        {
        ...
        where: 'id = ?',
        whereArgs: [bootstrapOwner.first['id']],
      );
      return true;
    }
  ...
  void _applySessionFromUserRow(Map<String, Object?> row) {
    _session = AppUserSession(
  ...
  Future<void> _applySessionFromUserRowAndReconcile(Map<String, Object?> row) async {
    _applySessionFromUserRow(row);
    await reconcileSessionOperationalScope();
  }

  /// يفتح جلسة المالك المحلي المرتبط ببريد الاشتراك (وليس حساب bootstrap «owner»).
  Future<void> enterSubscriptionOwnerSession({String? preferredEmail}) async {
    await ensureLocalOwnerForSubscription();
    final db = await _databaseService.database;
    var orgId = await OperationalScopeResolver.resolveOperationalOrganizationId(
      db,
    ...
    final email = preferredEmail?.trim().toLowerCase() ??
    ...
    if (email.contains('@')) {
      ...
      if (byEmail.isNotEmpty) {
        await _applySessionFromUserRowAndReconcile(byEmail.first);
        return;
      }
    }
    ...
    final rows = await db.rawQuery(
      ...
      WHERE organizationId = ?
        ...
        AND COALESCE(accountStatus, 'active') = 'active'
      ORDER BY
        CASE WHEN lower(trim(username)) = ? THEN 1 ELSE 0 END ASC,
        createdAt DESC
      LIMIT 1
      ''',
      [orgId, bootstrapOwnerUsername],
    ...
    _applySessionFromUserRow(rows.first);
    await reconcileSessionOperationalScope();
  }

  /// بعد تسجيل دخول/تسجيل القسيمة: يربط المالك المحلي ببريد المشترك ويفتح جلسة موظف.
  Future<bool> syncStaffSessionAfterVoucherAuth({
    String? email,
    String? password,
  }) async {
    try {
      ...
      assert(() {
        debugPrint('syncStaffSessionAfterVoucherAuth: $e\n$st');
        return true;
      }());
      return false;
    }
  ...
  Future<void> bootstrapDefaults() async {
    ...
    await ensureDefaultOwnerUserIfMissing();
  ...
  Future<AppUserSession?> login(
    ...
    for (var attempt = 0; attempt < 2; attempt++) {
      ...
      final pwdOk = PasswordCrypto.verifyPlainOrHash(password, storedPwd);
      if (!pwdOk) {
        if (attempt == 0) continue;
        return null;
      }
  ...
  Future<void> _pushTeamUserRowAfterSave(Map<String, Object?> row) async {
    if (!RemoteSignupConfig.teamUsersCrossDeviceSyncEnabled) return;
    if (!TeamUsersSyncService.isEligibleTeamUserRow(row)) return;
    final org = (row['organizationId'] as String?)?.trim() ?? '';
    if (org.isEmpty) return;
    final creds =
        await resolveSubscriptionApiCredentialsForOrganization(org);
    if (creds == null) {
      throw TeamUsersApiException('forbidden');
    }
    await TeamUsersSyncService(accountingService: this).pushUserRow(row);
  }
  ...
  static const bootstrapOwnerPassword = '1234';

  Future<void> ensureDefaultOwnerUserIfMissing() async {
    final db = await _databaseService.database;
    final orgRows =
        await db.query('organizations', orderBy: 'createdAt ASC', limit: 1);
    if (orgRows.isEmpty) return;
    ...
    if (ownerExists.isNotEmpty) return;
    ...
    await db.insert('users', {
      'id': _uuid.v4(),
      'organizationId': orgId,
      'branchId': br.first['id'] as String,
      'fullName': bootstrapOwnerDisplayName,
      'username': bootstrapOwnerUsername,
      'email': 'owner@legacy.mizapos',
      'phone': '',
      'dialCode': '+970',
      'password': PasswordCrypto.hash(bootstrapOwnerPassword),
      'role': 'owner',
      'accountStatus': 'active',
      'createdAt': now,
      'fromSignupRequestId': null,
  ...
  Future<void> enterDefaultOwnerSession() async {
    await ensureDefaultOwnerUserIfMissing();
    final db = await _databaseService.database;
    var orgId = await OperationalScopeResolver.resolveOperationalOrganizationId(
      db,
    ...
    await _applySessionFromUserRowAndReconcile(rows.first);
  }
  ...
  void _applyDistributorCloudFromUserRow(Map<String, Object?> row) {
    final untilRaw = (row['distributorCloudUntil'] as String?)?.trim();
  ...
  /// مزامنة صريحة لسحابة الموزّعين — قبل إضافة موزّع سحابة من الحاسوب.
  ...
  /// تجربة منظومة الموزّعين: بدون اشتراك POS/سحابة — حتى 5 أصناف على الشاحنة.
  static const int distributorTrialProductLimit = 5;
  static const int distributorTrialExpenseLimit = 3;

  bool get isDistributorFieldTrialMode {
    if (hasDistributorCloudAccess) return false;
    final now = DateTime.now();
    if (licenseGate.hasPaidCoverage(now)) return false;
    if (_voucherSubscriptionActiveOnDevice) return false;
    return true;
  }
  ...
  /// إدارة قائمة الموظفين: المالك، المحاسب، أو مدير الفرع — لا يشمل الزائر أو الكاشير.
  bool canManageUsers() {
  ...
  /// إعدادات المتجر — متاحة للجميع (زائر أو أي موظف).
  ...
  /// موزّعون سحابة فقط — للتوزيع الميداني (تحميل شاحنة، طلبات، تقارير).
  Future<List<Map<String, Object?>>> listCloudDistributors() async {
        ...
        .where(
          (u) =>
              (u['role'] ?? '').toString().trim().toLowerCase() ==
                  'distributor' &&
              parseDistributorCloudEnabled(u),
        )
        .map((e) => Map<String, Object?>.from(e))
        .toList(growable: false);
  }

  Future<Set<String>> cloudDistributorUserIds() async {
    final list = await listCloudDistributors();
    return list
        .map((u) => (u['id'] as String?)?.trim() ?? '')
  ...
  Future<void> addUser({
    ...
    if (inserted.isNotEmpty) {
      await _pushTeamUserRowAfterSave(inserted.first);
    }
  ...
  Future<void> updateUser({
    ...
    if (updatedRow.isNotEmpty) {
      await _pushTeamUserRowAfterSave(updatedRow.first);
    }
    if (RemoteSignupConfig.activationServerEnabled) {
  ...
  Future<List<Map<String, Object?>>> listProducts({
    ...
    DebugSessionLog.write(
      ...
      data: {
        ...
        'firstProductName':
            maps.isEmpty ? null : (maps.first['name'] ?? '').toString(),
      },
    );
    // #endregion
  ...
  /// تسجيل تعديل مخزون عبر مسار المعاملات (زيادة/نقصان موقّع).
  Future<String> createInventoryAdjustment({
    ...
    await _ensureTransactionOutboxReady();
    final s = _mustSession();
    final db = await _databaseService.database;
    final rows = await db.query(
      'products',
  ...
  /// تسجيل رصيد افتتاحي للمخزون عبر مسار المعاملات (كمية موجبة فقط).
  ...
  Future<void> updateProductDetails({
    ...
    final currentStockQty =
        (rows.first['stockQty'] as num?)?.toDouble() ?? 0.0;
    final nowService = isService;
    var targetQty = stockQty;
    if (nowService) {
      targetQty = 0;
    } else if (targetQty < 0) {
      throw Exception('الكمية لا يمكن أن تكون سالبة.');
    ...
    if (nowService && currentStockQty > 1e-9) {
      await createInventoryAdjustment(
        productId: productId,
        quantityDelta: -currentStockQty,
        adjustmentReason: 'correction',
      );
    } else if (!nowService && (targetQty - currentStockQty).abs() > 1e-9) {
      await setProductStockQuantity(
        productId: productId,
        stockQty: targetQty,
      );
    }
  ...
  Future<String?> findSaleInvoiceIdForFieldOrder({
    String? fieldOrderId,
    ...
    for (final marker in <String?>[
      ...
      final key = marker?.trim() ?? '';
      if (key.isEmpty) continue;
      final rows = await db.query(
        'salesInvoices',
        columns: ['id'],
        where:
            'organizationId = ? AND branchId = ? AND notes LIKE ? AND invoiceStatus != ?',
        whereArgs: [
          s.organizationId,
          s.branchId,
          '%طلب ميداني: $key%',
  ...
  Future<String> createSaleReturn({
    ...
    await db.transaction((txn) async {
        ...
        if (refund == 'credit') {
          ...
          if (origSaleStored != null) {
            final orig = await txn.query(
              ...
              columns: ['paymentType'],
              where: 'id = ? AND organizationId = ? AND branchId = ?',
              whereArgs: [origSaleStored, s.organizationId, s.branchId],
              limit: 1,
            );
            if (orig.isNotEmpty &&
      ...
      if (ledgerAdj && cid != null && cid.isNotEmpty && total != 0) {
        await _insertPartnerLedger(
          txn,
          partnerKind: 'customer',
          partnerId: cid,
          ...
          referenceId: returnId,
          amountSigned: -total,
          notes: notes.isEmpty ? 'مرتجع بيع' : notes,
          entryDate: now,
        );
      }
