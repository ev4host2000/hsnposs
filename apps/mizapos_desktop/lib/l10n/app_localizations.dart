...
class AppLocalizations {
  ...
  String get subscriptionPlanPriceShort => _t(
        '70 \$ · دفعة واحدة · جهازان',
        '\$70 · one-time · 2 devices',
      );

  /// عنوان فرعي أوضح في بطاقة الخطة.
  String get subscriptionPlanPriceHeadline => _t(
        '70 دولار — دفعة واحدة — جهازان (ويندوز + أندرويد)',
        'USD 70 — one-time — 2 devices (Windows + Android)',
      );

  /// شعار الخطّة الأساسي: «دفعة واحدة فقط — لا اشتراك شهري ولا سنوي».
  String get subscriptionPlanBadgeOneTime =>
      _t('دفعة واحدة فقط', 'One-time payment');
  ...
  String get subscriptionPlanPriceSuffix => _t('دفعة واحدة', 'one-time');

  String get subscriptionPlanIncludesTitle =>
      _t('يشمل الاشتراك:', 'Plan includes:');

  String get subscriptionPlanDevicesLabel => _t('جهازان (2)', '2 devices');
  ...
  String get subscriptionPlanFootnote => _t(
        'تفعيل عبر قسيمة Miza-XXXX-XXXX-XXXX بعد إنشاء الحساب.',
        'Activate by a Miza-XXXX-XXXX-XXXX voucher after creating your account.',
      );
  ...
  String get suppliersToolbarHintDetail => _t(
        'الشريط الأزرق: مورد جديد، ذمم الآجل، معاينة PDF، «اختر التقرير» (قائمة منسدلة يسار الشريط)، وأيقونة المزيد (الأرصدة الافتتاحية والنقدي، المتبقي). بعد اختيار التقرير حدّد الفترة ثم اعرضه؛ قد يُطلب اختيار مورد.',
        'Blue bar: new supplier, deferred AP, PDF preview, «Pick report» dropdown on the left, and more menu (opening balances & cash, outstanding). After picking a report, set the period; some reports ask you to pick a supplier.',
      );
  ...
  String get suppliersChipPdfPreview =>
      _t('معاينة PDF', 'PDF preview');
  ...
  String _activationWebOnlyPlain(String recipientName) => _en
      ? 'Hello ${recipientName.trim()},\n\n'
          'Your MizaPos account has been approved.\n\n'
          'Program price: US \$70 one-time. It covers the Windows desktop app and the Android app linked to the same email.\n\n'
      ...
      : 'مرحباً ${recipientName.trim()}،\n\n'
          'تمت الموافقة على حسابك في MizaPos.\n\n'
          'سعر البرنامج: 70 دولاراً أمريكياً للمرة الواحدة، ويشمل نسخة ويندوز ونسخة أندرويد المرتبطة بنفس البريد.\n\n'
...
<p style="margin:0 0 14px;color:#475569;">تمت الموافقة على حسابك. سعر البرنامج <strong>70 دولاراً أمريكياً للمرة الواحدة</strong> ويشمل نسخة ويندوز وأندرويد لنفس البريد.</p>
...
<p style="margin:18px 0 0;color:#94a3b8;font-size:13px;text-align:center;">Regards,<br/>MizaPos Team</p>
  ...
  String get settingsNoPermission => _t(
        'إعدادات المتجر متاحة لحساب المدير فقط.',
        'Store settings are available to the manager account only.',
      );
  ...
  String get settingsManagerOnlyBody => _t(
        'هذه الشاشة تفتح فقط لحساب المدير. سجّل الدخول بحساب المدير للوصول إليها.',
        'This screen is only available to the manager account. Sign in as the manager to access it.',
      );
  ...
  String get settingsErrProgramUnlockMismatch =>
      _t('كلمتا المرور غير متطابقتين.', 'Passwords do not match.');
  String get ownerDisplayNameAdmin => 'Admin';
  ...
  String get secDashTileShortcutsSub => _t(
        'أزرار خفيفة تحت كل مربّع للوصول السريع (سطح المكتب فقط)',
        'Light quick-action chips under each tile (desktop only)',
      );
  ...
  String get menuFieldOrders => _t('مبيعات الميدان', 'Field sales');

  String get menuDistributors => _t('الموزعون', 'Distributors');

  String get distributorsHubSubtitle => _t(
        'مبيعات الميدان، السيارات، والمخزون',
        'Field sales, vehicles, and inventory',
      );

  String get distributorsHubIntro => _t(
        'إدارة موزّعي الميدان من مكان واحد: اعتماد الطلبات، تتبّع السيارات، التحميل، وإدارة المرتجعات.',
        'Manage field distributors in one place: approve orders, track vehicles, load stock, and manage returns.',
      );

  String get distributorsHubFieldOrdersDesc => _t(
        'اعتماد أو رفض مبيعات الموزّعين الواردة من الميدان',
        'Approve or reject distributor sales from the field',
      );

  String get distributorsHubMovementsDesc => _t(
        'سجل تحميل وبيع وإرجاع بضاعة السيارة',
        'Log of truck load, sale, and return movements',
      );

  String get distributorsHubLoadDesc => _t(
        'نقل أصناف من المستودع إلى سيارة موزّع',
        'Move items from warehouse to a distributor truck',
      );
  ...
  String get fieldReturnsLocalReturnFailed => _t(
        'تعذّر إنشاء فاتورة المرتجع محلياً.',
        'Could not create the local return invoice.',
      );
  ...
  String get fieldReturnsOrderNotApproved => _t(
        'البيع الأصلي لم يُعتمد بعد — اعتمد المبيعة أولاً من «طلبات الميدان» ثم أعد اعتماد المرتجع.',
        'Original sale is not approved yet — approve the sale from field orders first, then approve the return.',
      );
  ...
  String get distributorsHubFieldExpensesDesc => _t(
        'اعتماد أو رفض مصروفات الموزّعين الواردة من الميدان',
        'Approve or reject distributor expenses from the field',
      );
  ...
  String get distributorsHubPublishDesc => _t(
        'مزامنة المنتجات والعملاء مع تطبيق الموزّع',
        'Sync products and customers to the distributor app',
      );

  String get distributorsHubPublishConfirm => _t(
        'سيتم نشر المنتجات والعملاء النشطين إلى خادم الميدان ليصلها تطبيق الموزّع.',
        'Active products and customers will be published to the field server for the distributor app.',
      );
  ...
  String get distributorsHubNotesDesc => _t(
        'مفكرة سريعة لملاحظات الميدان والموزّعين',
        'Quick notepad for field and distributor notes',
      );
  ...
  String get fieldCatalogPublishFailed => _t(
        'تعذّر نشر المخزون. تحقق من الاتصال وإعدادات الخادم.',
        'Could not publish inventory. Check connection and server settings.',
      );
  ...
  String get fieldOrdersUnavailable => _t(
        'مبيعات الميدان تتطلب اتصال خادم التفعيل وبريد اشتراك صالحاً.',
        'Field sales require the activation server and a valid subscription e-mail.',
      );

  String get fieldOrdersReviewerLoginRequired => _t(
        'سجّل دخول كمالك أو مدير فرع أو محاسب لاستخدام مبيعات الميدان ونشر المخزون.',
        'Sign in as owner, branch manager, or accountant to use field sales and publish inventory.',
      );
  ...
  String get fieldOrdersForbidden => _t(
        'لا صلاحية لعرض مبيعات هذه المؤسسة.',
        'No permission to view sales for this organization.',
      );

  String get fieldOrdersLoadFailed => _t(
        'تعذر تحميل المبيعات المعلّقة.',
        'Could not load pending sales.',
      );

  String get fieldOrdersEmpty => _t(
        'لا توجد مبيعات معلّقة من الموزّعين.',
        'No pending distributor sales.',
      );

  String get fieldOrdersEmptyFiltered => _t(
        'لا مبيعات معلّقة لهذا الموزّع.',
        'No pending sales for this distributor.',
      );

  String get fieldOrdersTapForDetails => _t(
        'اضغط لعرض الأصناف والإجماليات',
        'Tap to view items and totals',
      );

  String get fieldOrdersInvoiceTitle => _t(
        'فاتورة مبيعات الميدان',
        'Field sales invoice',
      );
  ...
  String get fieldOrdersApproveTitle => _t('اعتماد المبيعة', 'Approve sale');

  String fieldOrdersApproveBody(String customer) => _t(
        'اعتماد مبيعة العميل «$customer» وإنشاء فاتورة بيع؟',
        'Approve sale for «$customer» and create a sales invoice?',
      );
  ...
  String get fieldOrdersRejectTitle => _t('رفض المبيعة', 'Reject sale');
  ...
  String fieldOrdersApprovedOk(String invoiceId) => _t(
        'تم اعتماد المبيعة وإنشاء فاتورة البيع.',
        'Sale approved and invoice created.',
      );

  String get fieldOrdersRejectedOk => _t('تم رفض المبيعة.', 'Sale rejected.');
