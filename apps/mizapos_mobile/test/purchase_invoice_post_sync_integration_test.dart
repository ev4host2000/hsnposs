...
/// Purchase invoice post sync — requires backend on 127.0.0.1:8787.
void main() {
  ...
  Future<int> readInvoiceSyncSequence() async {
    ...
    final rows = await db.query(
      ...
      whereArgs: [companyId, branchId, PurchaseInvoiceSyncConstants.scopeKey],
    );
  ...
  Future<void> rewindInvoiceSyncSequence(int sequence) async {
    ...
    if (rows.isEmpty) {
      await db.insert('sync_meta', {
        ...
        'branch_id': branchId,
        'scope_key': PurchaseInvoiceSyncConstants.scopeKey,
  ...
  test('device A draft → post → push → cloud → pull → device B', () async {
    ...
    expect(
      movements.first['id'],
      PurchaseInvoicePostIds.stockMovementId(invoiceId, lineId),
    );
