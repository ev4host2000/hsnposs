...
/// Supplier payment sync performance — requires backend on 127.0.0.1:8787.
void main() {
  ...
  setUp(() async {
    ...
    transactionRegistry = SupplierPaymentSyncRegistry.createRegistered(
      apiClient: apiClient,
      config: config,
      databaseService: databaseService,
    );
  ...
  Future<Map<String, double>> benchmarkFullCycle(int count) async {
    ...
    for (final id in paymentIds) {
      await db.delete('supplierPayments', where: 'id = ?', whereArgs: [id]);
    }
    await db.delete('cashTransactions');
    await db.delete('partnerLedger');
  ...
  test('performance report — push + pull sync cycle', () async {
    if (!backendAvailable) {
      // ignore: avoid_print
      print('\n=== Supplier Payment Sync Benchmark SKIPPED (backend unavailable) ===\n');
      return;
    }
    ...
    // ignore: avoid_print
    print('\n=== Supplier Payment Sync Performance Report ===');
