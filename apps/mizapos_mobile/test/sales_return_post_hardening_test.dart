...
void main() {
  ...
  group('Sales return post hardening', () {
    ...
    test('transaction_version mismatch is rejected', () async {
      ...
      await db.transaction((txn) async {
        result = await SalesReturnPostingPipeline.create().run(
          PostingContext(
            aggregate: aggregate,
            txn: txn,
    ...
    test('post on already posted return with stale version fails', () async {
      final db = await databaseService.database;
      await db.transaction((txn) async {
        await SalesReturnPostingPipeline.create().run(
          PostingContext(
    ...
    test('crash recovery — replay pipeline after successful post is idempotent', () async {
      ...
      final movementId = SalesReturnPostIds.stockMovementId(returnId, returnLineId);
      final movements = await db.query(
        'stockMovements',
        where: 'id = ?',
    ...
    test('integrity stage rejects unbalanced accounting', () async {
      ...
      await db.transaction((txn) async {
        ...
        await SalesReturnValidationPostingStage().run(context);
        await SalesReturnInventoryPostingStage().run(context);
        await SalesReturnAccountingPostingStage().run(context);
        ...
        final result = await SalesReturnIntegrityPostingStage().run(context);
