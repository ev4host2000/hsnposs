...
import 'package:mizapos_mobile/services/cloud/sync/transactions/opening_stock_draft_apply_handler.dart';
...
/// Local sync-layer hardening for opening stock (no live backend).
void main() {
  ...
  Map<String, dynamic> draftEnvelope({
    ...
    return openingStockDraftPushEnvelope(
  ...
  group('Opening stock sync hardening', () {
    test('pull apply create upserts draft', () async {
      ...
      expect(header.first['openingStatus'], 'draft');
      expect(header.first['productId'], productId);
      expect(header.first['openingQuantity'], 5.0);
    });
    ...
    test('pull apply deferred when product missing', () async {
      ...
      expect(
        await db.query('openingStocks', where: 'id = ?', whereArgs: [openingStockId]),
        isEmpty,
      );
    });
    ...
    test('pull apply post applies stock movement exactly once on replay', () async {
      ...
      expect((await postService.postDraft(openingStockId: openingStockId)).ok, isTrue);
      ...
      expect(
        (await db.query(
          'stockMovements',
          where: 'id = ?',
          whereArgs: [OpeningStockPostIds.stockMovementId(openingStockId)],
        )).length,
        1,
      );
    });
    ...
    test('duplicate pull apply update upserts without duplicate rows', () async {
      ...
      expect(header.first['openingQuantity'], 8.0);
      expect(
        (await db.query(
          'openingStocks',
          where: 'id = ?',
    ...
    test('stable post outbox idempotency key on replay', () async {
      ...
      expect(
        (await service.postDraft(openingStockId: openingStockId)).idempotentReplay,
        isTrue,
      );
