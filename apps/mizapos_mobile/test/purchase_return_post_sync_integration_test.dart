...
/// Purchase return post sync — requires backend on 127.0.0.1:8787.
void main() {
  ...
  Future<void> pullReturnsOnly() {
    final workers = transactionRegistry.pullWorkers
        .where(
          (w) => w.repository.entityType == PurchaseReturnSyncConstants.entityType,
        )
        .toList();
  ...
  Future<int> readReturnSyncSequence() async {
    ...
    final rows = await db.query(
      ...
      whereArgs: [companyId, branchId, PurchaseReturnSyncConstants.scopeKey],
    );
  ...
  Future<void> rewindReturnSyncSequence(int sequence) async {
    ...
    if (rows.isEmpty) {
      await db.insert('sync_meta', {
        ...
        'branch_id': branchId,
        'scope_key': PurchaseReturnSyncConstants.scopeKey,
  ...
  test('device A draft → post → push → cloud → pull → device B', () async {
    ...
    await db.delete('purchaseReturns', where: 'id = ?', whereArgs: [returnId]);
    await db.update(
      'products',
      {'stockQty': 102.0},
      where: 'id = ?',
    ...
    final qtyRemoved = movements.fold<double>(
      0,
    ...
    expect(product.first['stockQty'], stockBeforePull! - qtyRemoved);
    expect(qtyRemoved, 2.0);
