...
/// Purchase return sync performance — requires backend on 127.0.0.1:8787.
void main() {
  ...
  setUp(() async {
    ...
    transactionRegistry = PurchaseReturnSyncRegistry.createRegistered(
      apiClient: apiClient,
      config: config,
      databaseService: databaseService,
    );
  ...
  Future<Map<String, double>> benchmarkFullCycle(int count) async {
    ...
    for (final id in returnIds) {
      ...
      await db.delete('purchaseReturnItems', where: 'returnId = ?', whereArgs: [id]);
    }
    await db.delete('stockMovements');
    await db.delete('partnerLedger');
  ...
  test('performance report — push + pull sync cycle', () async {
    ...
    // ignore: avoid_print
    print('\n=== Purchase Return Sync Performance Report ===');
    ...
    // ignore: avoid_print
    print('Compare with Purchase Invoice sync benchmark for baseline.');
    // ignore: avoid_print
