...
/// Local sync-layer hardening for purchase returns (no live backend).
void main() {
  ...
  Map<String, dynamic> draftEnvelope({
    ...
    return purchaseReturnDraftPushEnvelope(
  ...
  group('Purchase return sync hardening', () {
    ...
    test('pull apply deferred when supplier missing', () async {
      ...
      expect(await db.query('purchaseReturns', where: 'id = ?', whereArgs: [returnId]), isEmpty);
    });

    test('pull apply post decreases inventory exactly once on replay', () async {
      await applyPull('create', draftEnvelope());

      final postService = PurchaseReturnPostLocalService(
        databaseService: databaseService,
      );
      ...
      expect(
        (await db.query(
          'stockMovements',
          where: 'id = ?',
          whereArgs: [PurchaseReturnPostIds.stockMovementId(returnId, returnLineId)],
        )).length,
        1,
      );
    });
    ...
    test('pull apply cancel removes draft', () async {
      await applyPull('create', draftEnvelope());
      final cancelEnvelope = purchaseReturnDraftPushEnvelope(
        aggregateJson: purchaseReturnDraftAggregate(
          id: returnId,
          organizationId: companyId,
          branchId: branchId,
          createdBy: userId,
    ...
    test('stable post outbox idempotency key on replay', () async {
      ...
      final service = PurchaseReturnPostLocalService(databaseService: databaseService);
