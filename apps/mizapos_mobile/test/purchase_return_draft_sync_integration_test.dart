...
/// Purchase return draft sync — requires backend on 127.0.0.1:8787.
void main() {
  ...
  Future<void> pullReturnsOnly() {
    final workers = transactionRegistry.pullWorkers
        .where((w) => w.repository.entityType == PurchaseReturnSyncConstants.entityType)
        .toList();
  ...
  Map<String, dynamic> draftAggregate({
    ...
    return purchaseReturnDraftAggregate(
      id: returnId,
      organizationId: companyId,
      branchId: branchId,
      createdBy: userId,
  ...
  Future<void> enqueueDraft({
    ...
    final envelope = purchaseReturnDraftPushEnvelope(
  ...
  Future<void> pushOriginalInvoiceToCloud() async {
    await ensureTestSupplierOnCloud(
      databaseService: databaseService,
      storage: storage,
  ...
  test('device A create draft → push → device B pull → update → delete', () async {
    ...
    await db.delete('purchaseReturnItems', where: 'returnId = ?', whereArgs: [returnId]);
