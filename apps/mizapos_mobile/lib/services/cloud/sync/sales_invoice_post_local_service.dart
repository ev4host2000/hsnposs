...
import 'package:mizapos_mobile/services/cloud/sync/sales_invoice_post_payload.dart';
...
/// Executes local sales-invoice post and enqueues a single `post` outbox event.
class SalesInvoicePostLocalService {
  SalesInvoicePostLocalService({DatabaseService? databaseService})
  ...
  Future<PostingResult> postDraft({
    ...
    try {
      await db.transaction((txn) async {
        ...
        final pipeline = SalesInvoicePostingPipeline.create();
        ...
        envelope = salesInvoicePostPushEnvelope(
  ...
  Future<MapTransactionAggregate?> _loadDraftAggregate(
    ...
    final lines = await SalesInvoicePostDb.loadInvoiceLines(txn, invoiceId);
    final txnVersion =
  ...
  Map<String, dynamic> _buildPostAggregate(
    ...
    return salesInvoicePostAggregate(
      id: header.id,
      ...
      inventory: inventoryJsonFromStageData(stageData),
      accounting: accountingJsonFromStageData(stageData),
    );
  }
