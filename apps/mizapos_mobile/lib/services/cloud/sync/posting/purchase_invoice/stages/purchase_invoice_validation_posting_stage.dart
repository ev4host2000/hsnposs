...
class PurchaseInvoiceValidationPostingStage extends PostingStage {
  ...
  Future<PostingStageResult> run(PostingContext context) async {
    ...
    if (await PurchaseInvoicePostDb.hasPostMovementsForInvoice(txn, invoiceId)) {
      return const PostingStageResult.failure(
        code: 'post_effects_exist',
        message: 'Stock movements already exist for this invoice',
      );
    }

    if (!await PurchaseInvoicePostDb.partnerExists(txn, 'suppliers', supplierId)) {
      return const PostingStageResult.failure(
    ...
    for (final line in aggregateLines) {
      ...
      if (!await PurchaseInvoicePostDb.productExists(txn, productId)) {
        return PostingStageResult.failure(
  ...
  void _loadPlannedEffects(PostingContext context) {
    var inventory = PurchaseInvoicePostEffects.readInventory(context.aggregate);
    var accounting = PurchaseInvoicePostEffects.readAccounting(context.aggregate);
    ...
    if (inventory.isEmpty || accounting.isEmpty) {
      ...
      inventory = PurchaseInvoicePostEffects.buildInventory(
        invoiceId: header.id,
      ...
      accounting = PurchaseInvoicePostEffects.buildAccounting(
        invoiceId: header.id,
        organizationId: header.companyId,
