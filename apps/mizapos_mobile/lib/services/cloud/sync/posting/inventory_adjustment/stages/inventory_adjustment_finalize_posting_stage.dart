...
import 'package:mizapos_mobile/services/cloud/sync/posting/inventory_adjustment/inventory_adjustment_post_db.dart';

/// Sets adjustmentStatus=posted, posted_at, bumps transaction_version and row_version.
class InventoryAdjustmentFinalizePostingStage extends PostingStage {
  ...
  Future<PostingStageResult> run(PostingContext context) async {
    ...
    final finalized = await InventoryAdjustmentPostDb.finalizeAdjustment(
      context.txn,
    ...
    if (!finalized) {
      return const PostingStageResult.failure(
        ...
        message: 'Adjustment was already posted or modified concurrently',
      );
    }
