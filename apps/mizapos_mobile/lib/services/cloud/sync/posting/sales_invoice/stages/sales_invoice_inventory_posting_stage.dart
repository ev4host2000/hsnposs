...
/// Creates stock_movements and deducts inventory — one movement per line, idempotent.
class SalesInvoiceInventoryPostingStage extends PostingStage {
  ...
  Future<PostingStageResult> run(PostingContext context) async {
    ...
    try {
      for (final effect in inventory) {
        await SalesInvoicePostDb.applyInventoryEffect(
          context.txn,
          effect: effect,
    ...
    context.stageData['inventory_movements'] =
        SalesInvoicePostEffects.inventoryJson(inventory);
