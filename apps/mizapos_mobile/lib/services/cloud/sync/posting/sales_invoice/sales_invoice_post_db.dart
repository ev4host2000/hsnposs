...
/// SQLite helpers for sales-invoice post effects (idempotent).
class SalesInvoicePostDb {
  SalesInvoicePostDb._();
  ...
  static Future<bool> hasPostMovementsForInvoice(
    ...
    final rows = await txn.query(
      ...
      whereArgs: ['sale', invoiceId],
      limit: 1,
    );
    return rows.isNotEmpty;
  }

  /// Inserts movement if missing; deducts stock once per new movement.
  static Future<void> applyInventoryEffect(
    DatabaseExecutor txn, {
    required SalesInvoiceInventoryEffect effect,
  ...
  static Future<void> applyAccountingEffect(
    DatabaseExecutor txn, {
    required SalesInvoiceAccountingEffect effect,
    required String organizationId,
    required String branchId,
  }) async {
