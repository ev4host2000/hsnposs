...
Write-Host 'Seed supplier (FK parent)' -ForegroundColor Cyan
$supPush = Invoke-Api POST '/v1/sync/push/suppliers' @{
    company_id = $companyId; branch_id = $branchId; device_id = $deviceId
    batch_id = [guid]::NewGuid().ToString()
    events = @(@{
        entity_type = 'supplier'; entity_id = $supplierId; operation = 'create'
        outbox_id = [guid]::NewGuid().ToString()
        idempotency_key = "$deviceId`:$supplierId`:create:draft"
        client_row_version = 1
        payload_json = @{
            id = $supplierId; company_id = $companyId; branch_id = $branchId
            name = 'Draft Test Supplier'
        }
    })
} $headers
if ($supPush.Status -notin 200, 202) { throw "Supplier push failed: $($supPush.Raw)" }

Write-Host 'Push create draft payment' -ForegroundColor Cyan
$batchId = [guid]::NewGuid().ToString()
$createPayload = New-AggregateEnvelope -Operation 'create'
$pushCreate = Invoke-Api POST '/v1/sync/push/supplier-payments' @{
    company_id = $companyId; branch_id = $branchId; device_id = $deviceId; batch_id = $batchId
    events = @(@{
        entity_type = 'supplier_payment'; entity_id = $paymentId; operation = 'create'
        outbox_id = [guid]::NewGuid().ToString()
...
Write-Host 'Pull Supplier Payments' -ForegroundColor Cyan
$pull = Invoke-Api GET "/v1/sync/pull/supplier-payments?company_id=$companyId&branch_id=$branchId&since_sequence=0&limit=50" $null $headers
...
$pushUpdate = Invoke-Api POST '/v1/sync/push/supplier-payments' @{
    company_id = $companyId; branch_id = $branchId; device_id = $deviceId; batch_id = $batchId2
    events = @(@{
        entity_type = 'supplier_payment'; entity_id = $paymentId; operation = 'update'
        outbox_id = [guid]::NewGuid().ToString()
...
$pushCancel = Invoke-Api POST '/v1/sync/push/supplier-payments' @{
    company_id = $companyId; branch_id = $branchId; device_id = $deviceId; batch_id = $batchId3
    events = @(@{
        entity_type = 'supplier_payment'; entity_id = $paymentId; operation = 'cancel'
        outbox_id = [guid]::NewGuid().ToString()
...
Write-Host 'Supplier Payment draft sync OK' -ForegroundColor Green
