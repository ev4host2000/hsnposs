...
use MizaCloud\Modules\Sync\Validators\PurchaseInvoicesPushValidator;

final class PurchaseInvoicesSyncService extends AbstractTransactionSyncService
{
    private const ENTITY_SCOPE = 'purchase_invoices';

    public function __construct(
        private readonly PurchaseInvoicesSyncRepository $repository,
        BearerToken $bearer,
        Logger $logger,
    ) {
    ...
    public function push(array $payload, Request $request): array
    {
        return $this->pushTransactions(
            $payload,
            $request,
            new PurchaseInvoicesPushValidator($payload),
            self::ENTITY_SCOPE,
            PurchaseInvoicesSyncRepository::ENTITY_TYPE,
            fn (...$args) => $this->repository->applyPurchaseInvoiceDraftLww(...$args),
        );
    }
    ...
    protected function repository(): PurchaseInvoicesSyncRepository
    {
