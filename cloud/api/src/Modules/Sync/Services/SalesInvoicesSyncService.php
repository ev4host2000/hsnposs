...
use MizaCloud\Modules\Sync\Validators\SalesInvoicesPushValidator;

final class SalesInvoicesSyncService extends AbstractTransactionSyncService
{
    private const ENTITY_SCOPE = 'sales_invoices';

    public function __construct(
        private readonly SalesInvoicesSyncRepository $repository,
        BearerToken $bearer,
        Logger $logger,
    ) {
    ...
    public function push(array $payload, Request $request): array
    {
        return $this->pushTransactions(
            $payload,
            $request,
            new SalesInvoicesPushValidator($payload),
            self::ENTITY_SCOPE,
            SalesInvoicesSyncRepository::ENTITY_TYPE,
            fn (...$args) => $this->repository->applySalesInvoiceDraftLww(...$args),
        );
    }
    ...
    protected function repository(): SalesInvoicesSyncRepository
    {
