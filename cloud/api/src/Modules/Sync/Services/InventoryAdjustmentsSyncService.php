...
use MizaCloud\Modules\Sync\Validators\InventoryAdjustmentsPushValidator;

final class InventoryAdjustmentsSyncService extends AbstractTransactionSyncService
{
    private const ENTITY_SCOPE = 'inventory_adjustments';

    public function __construct(
        private readonly InventoryAdjustmentsSyncRepository $repository,
        BearerToken $bearer,
        Logger $logger,
    ) {
    ...
    public function push(array $payload, Request $request): array
    {
        return $this->pushTransactions(
            $payload,
            $request,
            new InventoryAdjustmentsPushValidator($payload),
            self::ENTITY_SCOPE,
            InventoryAdjustmentsSyncRepository::ENTITY_TYPE,
            fn (...$args) => $this->repository->applyInventoryAdjustmentDraftLww(...$args),
        );
    }
    ...
    protected function repository(): InventoryAdjustmentsSyncRepository
    {
