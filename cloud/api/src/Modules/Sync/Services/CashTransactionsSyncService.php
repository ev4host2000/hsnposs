...
use MizaCloud\Modules\Sync\Validators\CashTransactionsPushValidator;
use MizaCloud\Modules\Sync\Validators\CatalogPullValidator;

final class CashTransactionsSyncService extends AbstractCatalogSyncService
{
    private const ENTITY_SCOPE = 'cash_transactions';

    public function __construct(
        private readonly CashTransactionsSyncRepository $repository,
        BearerToken $bearer,
        Logger $logger,
    ) {
    ...
    public function push(array $payload, Request $request): array
    {
        ...
        return $this->pushEntities(
            $payload,
            $request,
            new CashTransactionsPushValidator($payload),
            self::ENTITY_SCOPE,
            CashTransactionsSyncRepository::ENTITY_TYPE,
            function (
                string $companyId,
                string $branchId,
                string $entityId,
                ...
                return $this->repository->applyCashTransactionLww(
                    $companyId,
                    $branchId,
                    $entityId,
                    $operation,
    ...
    protected function repository(): CashTransactionsSyncRepository
    {
