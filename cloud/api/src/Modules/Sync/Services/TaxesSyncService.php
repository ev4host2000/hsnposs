...
final class TaxesSyncService extends AbstractCatalogSyncService
{
    private const ENTITY_SCOPE = 'taxes';

    public function __construct(
        private readonly TaxesSyncRepository $repository,
        BearerToken $bearer,
        Logger $logger,
    ) {
    ...
    public function push(array $payload, Request $request): array
    {
        return $this->pushEntities(
            $payload,
            $request,
            new TaxesPushValidator($payload),
            self::ENTITY_SCOPE,
            TaxesSyncRepository::ENTITY_TYPE,
            fn (...$args) => $this->repository->applyTaxLww(...$args),
        );
    }
