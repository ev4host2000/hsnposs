...
function New-DraftEnvelope {
    param([int]$TransactionVersion = 0, [int]$RowVersion = 1, [double]$OpeningQuantity = 5)
    return @{
...
Write-Host '=== Opening Stock Post Sync Test ===' -ForegroundColor Cyan
$token = Get-Token
...
$prodPush = Invoke-Api -Method POST -Path '/v1/sync/push/products' -Headers $auth -Body @{
    ...
    events = @(@{
        ...
        idempotency_key = "$deviceId`:$productId`:create:os-post"
        payload_json = @{
            id = $productId
            company_id = $companyId
            branch_id = $branchId
...
$draftPush = Invoke-Api -Method POST -Path '/v1/sync/push/opening-stocks' -Headers $auth -Body @{
    company_id = $companyId
    branch_id = $branchId
    device_id = $deviceId
    batch_id = [guid]::NewGuid().ToString()
    events = @(@{
        ...
        entity_id = $openingStockId
        operation = 'create'
        client_row_version = 1
        idempotency_key = "$deviceId`:$openingStockId`:create:test"
...
$postPush = Invoke-Api -Method POST -Path '/v1/sync/push/opening-stocks' -Headers $auth -Body @{
    company_id = $companyId
    branch_id = $branchId
    device_id = $deviceId
    batch_id = [guid]::NewGuid().ToString()
    events = @(@{
        ...
        idempotency_key = "$deviceId`:$openingStockId`:post"
...
$postPush2 = Invoke-Api -Method POST -Path '/v1/sync/push/opening-stocks' -Headers $auth -Body @{
    company_id = $companyId
    branch_id = $branchId
    device_id = $deviceId
    batch_id = [guid]::NewGuid().ToString()
