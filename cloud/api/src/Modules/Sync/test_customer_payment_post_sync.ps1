...
Write-Host '=== Customer Payment Post Sync Test ===' -ForegroundColor Cyan
$token = Get-Token
...
$draftPush = Invoke-Api -Method POST -Path '/v1/sync/push/customer-payments' -Headers $auth -Body @{
    company_id = $companyId
    branch_id = $branchId
    device_id = $deviceId
    batch_id = [guid]::NewGuid().ToString()
...
$postPush = Invoke-Api -Method POST -Path '/v1/sync/push/customer-payments' -Headers $auth -Body @{
    company_id = $companyId
    branch_id = $branchId
    device_id = $deviceId
    batch_id = [guid]::NewGuid().ToString()
...
$postPush2 = Invoke-Api -Method POST -Path '/v1/sync/push/customer-payments' -Headers $auth -Body @{
    company_id = $companyId
    branch_id = $branchId
    device_id = $deviceId
    batch_id = [guid]::NewGuid().ToString()
