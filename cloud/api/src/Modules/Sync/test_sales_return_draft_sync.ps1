...
Write-Host 'Seed original sales invoice (FK parent)' -ForegroundColor Cyan
$origPush = Invoke-Api POST '/v1/sync/push/sales-invoices' @{
    company_id = $companyId; branch_id = $branchId; device_id = $deviceId
    batch_id = [guid]::NewGuid().ToString()
    events = @(@{
        entity_type = 'sales_invoice'; entity_id = $originalInvoiceId; operation = 'create'
...
$pushCreate = Invoke-Api POST '/v1/sync/push/sales-returns' @{
    company_id = $companyId; branch_id = $branchId; device_id = $deviceId; batch_id = $batchId
    events = @(@{
        entity_type = 'sales_return'; entity_id = $returnId; operation = 'create'
        outbox_id = [guid]::NewGuid().ToString()
...
Write-Host 'Pull Sales Returns' -ForegroundColor Cyan
$pull = Invoke-Api GET "/v1/sync/pull/sales-returns?company_id=$companyId&branch_id=$branchId&since_sequence=0&limit=50" $null $headers
...
$pushUpdate = Invoke-Api POST '/v1/sync/push/sales-returns' @{
    company_id = $companyId; branch_id = $branchId; device_id = $deviceId; batch_id = $batchId2
    events = @(@{
        entity_type = 'sales_return'; entity_id = $returnId; operation = 'update'
        outbox_id = [guid]::NewGuid().ToString()
...
$pushCancel = Invoke-Api POST '/v1/sync/push/sales-returns' @{
    company_id = $companyId; branch_id = $branchId; device_id = $deviceId; batch_id = $batchId3
    events = @(@{
        entity_type = 'sales_return'; entity_id = $returnId; operation = 'cancel'
        outbox_id = [guid]::NewGuid().ToString()
...
Write-Host 'Sales Return draft sync OK' -ForegroundColor Green
