...
final class PurchaseReturnsPushValidator extends TransactionPushValidator
{
    ...
    private function validatePostPayload(array $payload, string $prefix): array
    {
        ...
        $originalInvoiceId = (string) ($header['original_invoice_id'] ?? '');
        if ($originalInvoiceId === '' || !Uuid::isValid($originalInvoiceId)) {
            $errors[] = "{$prefix}.payload_json.aggregate.header.original_invoice_id:Valid original_invoice_id is required for post";
        }
    ...
    protected function validateAggregateExtras(array $aggregate, string $prefix): array
    {
        ...
        $originalInvoiceId = (string) ($header['original_invoice_id'] ?? '');
        if ($originalInvoiceId === '' || !Uuid::isValid($originalInvoiceId)) {
            $errors[] = "{$prefix}.payload_json.aggregate.header.original_invoice_id:Valid original_invoice_id required";
        }
