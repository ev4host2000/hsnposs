...
final class CashTransactionsPushValidator extends SyncValidator
{
    ...
    private function validateEvent(array $event, int $index): array
    {
        ...
        if (($event['entity_type'] ?? '') !== 'cash_transaction') {
            $errors[] = "{$prefix}.entity_type:Only cash_transaction entity_type is supported";
        }

        $operation = (string) ($event['operation'] ?? '');
        if ($operation !== 'create') {
            $errors[] = "{$prefix}.operation:Only create is supported for cash_transaction";
        }
        ...
        if ($type !== 'in' && $type !== 'out') {
            $errors[] = "{$prefix}.payload_json.transaction_type:transaction_type must be in or out";
        }
        ...
        if (!is_string($payload['description'] ?? null)) {
            $errors[] = "{$prefix}.payload_json.description:description is required";
        }

        if (!is_string($payload['transaction_date'] ?? null) || trim((string) $payload['transaction_date']) === '') {
            $errors[] = "{$prefix}.payload_json.transaction_date:transaction_date is required";
        }
        ...
        if ($referenceId !== null && $referenceId !== '' && (!is_string($referenceId) || !Uuid::isValid($referenceId))) {
            $errors[] = "{$prefix}.payload_json.reference_id:reference_id must be a valid UUID when set";
        }
