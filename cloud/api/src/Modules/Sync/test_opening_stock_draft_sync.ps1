...
$openingStockId = [guid]::NewGuid().ToString()
    ...
    return @{
        ...
        aggregate = @{
            header = @{
                ...
                opening_date = (Get-Date).ToUniversalTime().ToString('yyyy-MM-dd')
                notes = $Notes
...
$prodPush = Invoke-Api POST '/v1/sync/push/products' @{
    ...
    events = @(@{
        ...
        idempotency_key = "$deviceId`:$productId`:create:os-test"
        client_row_version = 1
        payload_json = @{
            id = $productId
            company_id = $companyId
            branch_id = $branchId
            name = 'Opening Stock Test Product'
            sku = 'OS-TEST-001'
            stock_qty = 10
            sale_price = 25
        }
...
Write-Host 'Push create draft opening stock' -ForegroundColor Cyan
$batchId = [guid]::NewGuid().ToString()
$createPayload = New-AggregateEnvelope -Operation 'create'
$pushCreate = Invoke-Api POST '/v1/sync/push/opening-stocks' @{
    company_id = $companyId; branch_id = $branchId; device_id = $deviceId; batch_id = $batchId
    events = @(@{
        entity_type = 'opening_stock'; entity_id = $openingStockId; operation = 'create'
        outbox_id = [guid]::NewGuid().ToString()
        idempotency_key = "$deviceId`:$openingStockId`:create:$batchId"
...
Write-Host 'Pull Opening Stocks' -ForegroundColor Cyan
$pull = Invoke-Api GET "/v1/sync/pull/opening-stocks?company_id=$companyId&branch_id=$branchId&since_sequence=0&limit=50" $null $headers
...
Write-Host 'Push update draft opening stock' -ForegroundColor Cyan
$batchId2 = [guid]::NewGuid().ToString()
$updatePayload = New-AggregateEnvelope -Operation 'update' -TransactionVersion 1 -RowVersion 2 -Notes 'Draft v2' -OpeningQuantity 8
$pushUpdate = Invoke-Api POST '/v1/sync/push/opening-stocks' @{
    company_id = $companyId; branch_id = $branchId; device_id = $deviceId; batch_id = $batchId2
    events = @(@{
        entity_type = 'opening_stock'; entity_id = $openingStockId; operation = 'update'
        outbox_id = [guid]::NewGuid().ToString()
        idempotency_key = "$deviceId`:$openingStockId`:update:$batchId2"
...
Write-Host 'Push cancel draft opening stock' -ForegroundColor Cyan
...
$pushCancel = Invoke-Api POST '/v1/sync/push/opening-stocks' @{
    company_id = $companyId; branch_id = $branchId; device_id = $deviceId; batch_id = $batchId3
    events = @(@{
        entity_type = 'opening_stock'; entity_id = $openingStockId; operation = 'cancel'
        outbox_id = [guid]::NewGuid().ToString()
        idempotency_key = "$deviceId`:$openingStockId`:cancel:$batchId3"
...
Write-Host 'Opening Stock draft sync OK' -ForegroundColor Green
