...
/**
 * Post handler for sales invoices — inventory + accounting + finalize.
 */
trait SalesInvoicePostLww
{
    /**
     * @param array<string, mixed> $payloadJson
     * @return array<string, mixed>
     */
    protected function applySalesInvoicePost(
        string $companyId,
        string $branchId,
        string $entityId,
        array $payloadJson,
        ...
        if ($dbStatus === 'posted') {
            ...
            throw new HttpException('already_posted', 'Invoice already posted', 409);
        }

        if ($dbStatus !== 'draft') {
            throw new HttpException('invoice_not_editable', 'Only draft invoices can be posted', 409);
        }
        ...
        if ($customerId === null || !Uuid::isValid($customerId)) {
            throw new HttpException('validation_error', 'customer_id is required for post', 400);
        }
        ...
        foreach ($inventory as $movement) {
            ...
            $this->applyStockMovementIdempotent(
                ...
                (string) ($movement['reference_type'] ?? 'sale'),
        ...
        foreach ($accounting as $entry) {
            ...
            $this->applyPartnerLedgerIdempotent(
                ...
                (string) ($entry['entry_type'] ?? 'sale_post_dr'),
        ...
        $hasPostedAt = $this->salesInvoiceHasPostedAt();
        ...
        $returnCols = 'row_version, invoice_status';
        if ($hasTxnVersion) {
            $returnCols = 'row_version, transaction_version, invoice_status';
        }
        ...
        if (!is_array($saved)) {
            ...
            if ($refetched !== null && (string) ($refetched['invoice_status'] ?? '') === 'posted') {
                return $this->buildPostAggregateEnvelope(
                    $companyId,
                    $branchId,
                    $entityId,
            ...
            throw new HttpException('posting_conflict', 'Invoice post conflict — draft lock failed', 409);
        }
    ...
    protected function applyStockMovementIdempotent(
        ...
        if ($newStock < -1e-9) {
            throw new HttpException('insufficient_stock', 'Insufficient stock for post', 409);
        }
    ...
    protected function salesInvoiceHasPostedAt(): bool
    {
        static $cached = null;
        if ($cached !== null) {
            return $cached;
