...
/**
 * Post handler for sales returns — inventory in + reversed sale accounting + finalize.
 */
trait SalesReturnPostLww
{
    /**
     * @param array<string, mixed> $payloadJson
     * @return array<string, mixed>
     */
    protected function applySalesReturnPost(
        string $companyId,
        string $branchId,
        string $entityId,
        array $payloadJson,
        ...
        foreach ($inventory as $movement) {
            ...
            $this->applyStockMovementIdempotent(
                ...
                (string) ($movement['reference_type'] ?? 'sale_return'),
        ...
        foreach ($accounting as $entry) {
            ...
            $this->applyPartnerLedgerIdempotent(
                ...
                (string) ($entry['reference_type'] ?? 'sale_return'),
        ...
        $hasPostedAt = $this->salesReturnHasPostedAt();
        ...
        $stmt = $pdo->prepare(
            'UPDATE sales_returns SET ' . implode(', ', $setParts) . '
        ...
        if (!is_array($saved)) {
            $refetched = $this->fetchSalesReturnRow($companyId, $entityId);
    ...
    protected function salesReturnHasPostedAt(): bool
    {
        static $cached = null;
        if ($cached !== null) {
            return $cached;
