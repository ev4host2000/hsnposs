...
/**
 * Draft-only LWW for supplier payments — header + metadata, no lines/accounting/cash.
 */
trait SupplierPaymentDraftLww
{
    use SupplierPaymentPostLww;
    ...
    protected function applyDraftLww(
        ...
        if ($operation === 'post') {
            return $this->applySupplierPaymentPost(
                $companyId,
                $branchId,
                $entityId,
                $payloadJson,
        ...
        if (in_array($operation, ['cancel', 'delete'], true)) {
            return $this->cancelSupplierPaymentDraft(
                $companyId,
                $branchId,
                $entityId,
                $clientRowVersion,
        ...
        if ($hasTxnVersion) {
            $sql = 'INSERT INTO supplier_payments (
                        id, company_id, branch_id, supplier_id, amount, payment_date,
                    ...
                    ON CONFLICT (id) DO UPDATE SET
                        ...
                        origin_device_id = COALESCE(EXCLUDED.origin_device_id, supplier_payments.origin_device_id),
                        transaction_version = GREATEST(supplier_payments.transaction_version, EXCLUDED.transaction_version),
                        row_version = GREATEST(supplier_payments.row_version, EXCLUDED.row_version),
                        updated_at = now(),
                        deleted_at = NULL
        ...
        if (!is_array($saved)) {
            throw new RuntimeException('Supplier payment upsert failed');
        }
     ...
     * @return array<string, mixed>
     */
    protected function cancelSupplierPaymentDraft(
        string $companyId,
        string $branchId,
        string $entityId,
        int $clientRowVersion,
        ...
        $deleted = $this->softDeleteRow('supplier_payments', $companyId, $entityId, $clientRowVersion);
    ...
    protected function fetchSupplierPaymentRow(string $companyId, string $entityId): ?array
    {
        $txnCol = $this->supplierPaymentHasTransactionVersion() ? ', transaction_version' : '';
        $postedCol = $this->supplierPaymentHasPostedAt() ? ', posted_at' : '';
