...
/**
 * Post handler for customer payments — partner ledger + cash + finalize.
 */
trait CustomerPaymentPostLww
{
    /**
     * @param array<string, mixed> $payloadJson
     * @return array<string, mixed>
     */
    protected function applyCustomerPaymentPost(
        string $companyId,
        string $branchId,
        string $entityId,
        array $payloadJson,
        ...
        if ($dbStatus === 'posted') {
            if ($headerTxnVersion <= $dbTxnVersion) {
                return $this->buildCustomerPaymentPostAggregateEnvelope(
                    $companyId,
                    $branchId,
                    $entityId,
                    $header,
        ...
        foreach ($accounting as $entry) {
            ...
            $this->applyPartnerLedgerIdempotent(
                ...
                (string) ($entry['entry_type'] ?? 'customer_payment'),
                (string) ($entry['reference_type'] ?? 'customer_payment'),
        ...
        foreach ($cash as $txn) {
            ...
            $this->applyCashTransactionIdempotent(
                ...
                $branchId,
                $txnId,
                (string) ($txn['transaction_type'] ?? 'in'),
                (float) ($txn['amount'] ?? 0),
                (string) ($txn['description'] ?? ''),
                (string) ($txn['reference_type'] ?? 'customer_payment'),
        ...
        $hasPostedAt = $this->customerPaymentHasPostedAt();
        ...
        $stmt = $pdo->prepare(
            'UPDATE customer_payments SET ' . implode(', ', $setParts) . '
        ...
        if (!is_array($saved)) {
            $refetched = $this->fetchCustomerPaymentRow($companyId, $entityId);
     ...
     * @return array<string, mixed>
     */
    protected function buildCustomerPaymentPostAggregateEnvelope(
        string $companyId,
        string $branchId,
        string $entityId,
        array $header,
    ...
    protected function customerPaymentHasPostedAt(): bool
    {
        static $cached = null;
        if ($cached !== null) {
            return $cached;
    ...
    protected function customerPaymentHasTransactionVersion(): bool
    {
        static $cached = null;
        if ($cached !== null) {
            return $cached;
