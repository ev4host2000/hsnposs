...
/**
 * Draft-only LWW for Purchase invoices — header + lines + metadata, no inventory/accounting.
 */
trait PurchaseInvoiceDraftLww
{
    ...
    protected function applyDraftLww(
        ...
        if (in_array($operation, ['cancel', 'delete'], true)) {
            return $this->cancelPurchaseInvoiceDraft(
                $companyId,
                $branchId,
                $entityId,
                $clientRowVersion,
        ...
        if ($hasTxnVersion) {
            $sql = 'INSERT INTO purchase_invoices (
                        id, company_id, branch_id, supplier_id, invoice_date, invoice_status,
                        ...
                        :id, :company_id, :branch_id, :supplier_id, :invoice_date, \'draft\',
                        :payment_type, :line_subtotal, :discount_amount, :tax_percent, :total, :paid_amount,
                        :notes, :created_by_user_id, :origin_device_id, :transaction_version,
                    ...
                    ON CONFLICT (id) DO UPDATE SET
                        ...
                        origin_device_id = COALESCE(EXCLUDED.origin_device_id, purchase_invoices.origin_device_id),
                        transaction_version = GREATEST(purchase_invoices.transaction_version, EXCLUDED.transaction_version),
                        row_version = GREATEST(purchase_invoices.row_version, EXCLUDED.row_version),
                        updated_at = now(),
                        deleted_at = NULL
        ...
        if (!is_array($saved)) {
            throw new RuntimeException('Purchase invoice upsert failed');
        }

        $this->replacePurchaseInvoiceLines($pdo, $companyId, $branchId, $entityId, $lines);
     ...
     * @return array<string, mixed>
     */
    protected function cancelPurchaseInvoiceDraft(
        string $companyId,
        string $branchId,
        string $entityId,
        int $clientRowVersion,
        ...
        $deleted = $this->softDeleteRow('purchase_invoices', $companyId, $entityId, $clientRowVersion);
    ...
    protected function fetchPurchaseInvoiceRow(string $companyId, string $entityId): ?array
    {
        $txnCol = $this->purchaseInvoiceHasTransactionVersion() ? ', transaction_version' : '';
     ...
     */
    protected function replacePurchaseInvoiceLines(
        \PDO $pdo,
        string $companyId,
        string $branchId,
        string $invoiceId,
        ...
        $delete = $pdo->prepare('DELETE FROM purchase_invoice_items WHERE invoice_id = :invoice_id');
    ...
    protected function purchaseInvoiceHasTransactionVersion(): bool
    {
        static $cached = null;
        if ($cached !== null) {
            return $cached;
