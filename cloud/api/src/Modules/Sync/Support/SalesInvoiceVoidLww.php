...
/**
 * Void handler for posted sales invoices — reverse stock + ledger, status=void.
 * Draft cancel remains cancelSalesInvoiceDraft (soft-delete only).
 */
trait SalesInvoiceVoidLww
{
    /**
     * @param array<string, mixed> $payloadJson
     * @return array<string, mixed>
     */
    protected function applySalesInvoiceVoid(
        string $companyId,
        string $branchId,
        string $entityId,
        array $payloadJson,
        ...
        if ($dbStatus === 'void') {
            if ($headerTxnVersion <= $dbTxnVersion) {
                // Idempotent void may still carry cash reverse for other devices.
                ...
                if ($cash !== []) {
                    ...
                    foreach ($cash as $cashRow) {
                        ...
                        $this->applyVoidCashTransactionIdempotent(
                            $pdo,
                            $companyId,
                            $branchId,
                            $cashId,
                            ...
                            (string) ($cashRow['reference_type'] ?? 'sale_void'),
                ...
                return $this->buildVoidAggregateEnvelope(
                    $companyId,
                    $branchId,
                    $entityId,
                    $header,
        ...
        if ($customerId === null || !Uuid::isValid($customerId)) {
            throw new HttpException('validation_error', 'customer_id is required for void', 400);
        }
        ...
        foreach ($inventory as $movement) {
            ...
            $this->applySalesVoidStockMovement(
                $pdo,
                $companyId,
                $branchId,
                $movementId,
                ...
                (string) ($movement['movement_type'] ?? 'in'),
                (string) ($movement['reference_type'] ?? 'sale_void'),
        ...
        foreach ($accounting as $entry) {
            ...
            $this->applyPartnerLedgerIdempotent(
                ...
                $branchId,
                $entryId,
                (string) ($entry['partner_kind'] ?? 'customer'),
                (string) ($entry['partner_id'] ?? ''),
                (string) ($entry['entry_type'] ?? 'sale_void'),
                (string) ($entry['reference_type'] ?? 'sale'),
        ...
        $stmt = $pdo->prepare(
            'UPDATE sales_invoices SET ' . implode(', ', $setParts) . '
        ...
        if (!is_array($saved)) {
            $refetched = $this->fetchSalesInvoiceRow($companyId, $entityId);
    ...
    protected function applySalesVoidStockMovement(
        \PDO $pdo,
        string $companyId,
        string $branchId,
        string $movementId,
        ...
        // Sales void restores stock (in). Reject unexpected out on void.
        $type = strtolower(trim($movementType));
        if ($type !== 'in') {
            throw new HttpException('validation_error', 'Sales void stock movement must be in', 400);
        }
     ...
     * @return array<string, mixed>
     */
    protected function buildVoidAggregateEnvelope(
        string $companyId,
        string $branchId,
        string $entityId,
        array $header,
    ...
    protected function applyVoidCashTransactionIdempotent(
        \PDO $pdo,
        string $companyId,
        string $branchId,
