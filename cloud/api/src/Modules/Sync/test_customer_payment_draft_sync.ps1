...
Write-Host 'Seed customer (FK parent)' -ForegroundColor Cyan
$custPush = Invoke-Api POST '/v1/sync/push/customers' @{
    company_id = $companyId; branch_id = $branchId; device_id = $deviceId
    batch_id = [guid]::NewGuid().ToString()
    events = @(@{
        entity_type = 'customer'; entity_id = $customerId; operation = 'create'
        outbox_id = [guid]::NewGuid().ToString()
        idempotency_key = "$deviceId`:$customerId`:create:draft"
        client_row_version = 1
        payload_json = @{
            id = $customerId; company_id = $companyId; branch_id = $branchId
            name = 'Draft Test Customer'; credit_limit = 0
        }
    })
} $headers
if ($custPush.Status -notin 200, 202) { throw "Customer push failed: $($custPush.Raw)" }

Write-Host 'Push create draft payment' -ForegroundColor Cyan
$batchId = [guid]::NewGuid().ToString()
$createPayload = New-AggregateEnvelope -Operation 'create'
$pushCreate = Invoke-Api POST '/v1/sync/push/customer-payments' @{
    company_id = $companyId; branch_id = $branchId; device_id = $deviceId; batch_id = $batchId
    events = @(@{
        entity_type = 'customer_payment'; entity_id = $paymentId; operation = 'create'
        outbox_id = [guid]::NewGuid().ToString()
        idempotency_key = "$deviceId`:$paymentId`:create:$batchId"
...
Write-Host 'Pull Customer Payments' -ForegroundColor Cyan
$pull = Invoke-Api GET "/v1/sync/pull/customer-payments?company_id=$companyId&branch_id=$branchId&since_sequence=0&limit=50" $null $headers
...
Write-Host 'Push update draft payment' -ForegroundColor Cyan
$batchId2 = [guid]::NewGuid().ToString()
$updatePayload = New-AggregateEnvelope -Operation 'update' -TransactionVersion 1 -RowVersion 2 -Notes 'Draft v2' -Amount 150
$pushUpdate = Invoke-Api POST '/v1/sync/push/customer-payments' @{
    company_id = $companyId; branch_id = $branchId; device_id = $deviceId; batch_id = $batchId2
    events = @(@{
        entity_type = 'customer_payment'; entity_id = $paymentId; operation = 'update'
        outbox_id = [guid]::NewGuid().ToString()
        idempotency_key = "$deviceId`:$paymentId`:update:$batchId2"
...
Write-Host 'Push cancel draft payment' -ForegroundColor Cyan
...
$pushCancel = Invoke-Api POST '/v1/sync/push/customer-payments' @{
    company_id = $companyId; branch_id = $branchId; device_id = $deviceId; batch_id = $batchId3
    events = @(@{
        entity_type = 'customer_payment'; entity_id = $paymentId; operation = 'cancel'
        outbox_id = [guid]::NewGuid().ToString()
        idempotency_key = "$deviceId`:$paymentId`:cancel:$batchId3"
...
Write-Host 'Customer Payment draft sync OK' -ForegroundColor Green
