...
$pushCreate = Invoke-Api POST '/v1/sync/push/purchase-invoices' @{
    company_id = $companyId; branch_id = $branchId; device_id = $deviceId; batch_id = $batchId
    events = @(@{
        entity_type = 'purchase_invoice'; entity_id = $invoiceId; operation = 'create'
...
Write-Host 'Pull Purchase Invoices' -ForegroundColor Cyan
$pull = Invoke-Api GET "/v1/sync/pull/purchase-invoices?company_id=$companyId&branch_id=$branchId&since_sequence=0&limit=50" $null $headers
...
$pushUpdate = Invoke-Api POST '/v1/sync/push/purchase-invoices' @{
    company_id = $companyId; branch_id = $branchId; device_id = $deviceId; batch_id = $batchId2
    events = @(@{
        entity_type = 'purchase_invoice'; entity_id = $invoiceId; operation = 'update'
        outbox_id = [guid]::NewGuid().ToString()
...
$pushCancel = Invoke-Api POST '/v1/sync/push/purchase-invoices' @{
    company_id = $companyId; branch_id = $branchId; device_id = $deviceId; batch_id = $batchId3
    events = @(@{
        entity_type = 'purchase_invoice'; entity_id = $invoiceId; operation = 'cancel'
        outbox_id = [guid]::NewGuid().ToString()
...
Write-Host 'REST list Purchase Invoices (draft filter)' -ForegroundColor Cyan
$listPath = "/v1/purchase-invoices?branch_id=$branchId" + '&status=draft'
$list = Invoke-Api GET $listPath $null @{
    Authorization = "Bearer $userToken"; 'X-Company-ID' = $companyId; 'X-Branch-ID' = $branchId
}
if ($list.Status -ne 200) { throw "REST list failed: $($list.Raw)" }

Write-Host 'Purchase Invoice draft sync OK' -ForegroundColor Green
